...
namespace Sln_Lidermax.Controllers
{
    ...
    public class HojasRutaController : Controller
    {
        ...
        public async Task<IActionResult> ExportarExcelPorHojaRuta(int docEntryHojaRuta, int docNumHojaRuta)
        {
            ...
            ws.Cell(1, 5).Value = "Facturas";
            ws.Cell(1, 6).Value = "OrdenCompra";
            ws.Cell(1, 7).Value = "Ruc";
            ws.Cell(1, 8).Value = "Direccion1";
            ws.Cell(1, 9).Value = "Direccion2";
            ws.Cell(1, 10).Value = "Departamento1";
            ws.Cell(1, 11).Value = "Departamento2";
            ws.Cell(1, 12).Value = "PersonaRecojo";
            ws.Cell(1, 13).Value = "Documento";
            ws.Cell(1, 14).Value = "Telefono";
            ws.Cell(1, 15).Value = "Cajas";
            ws.Cell(1, 16).Value = "Peso";
            ws.Cell(1, 17).Value = "Transportista";
            ws.Cell(1, 18).Value = "ModoEnvio";
            ...
            foreach (var x in lista)
            {
                ...
                ws.Cell(row, 5).Value = x.Factura;
                ws.Cell(row, 6).Value = x.DocNumTicket;
                ws.Cell(row, 7).Value = x.CardCode;
                ws.Cell(row, 8).Value = x.Calle1;
                ws.Cell(row, 9).Value = x.Calle2;
                ws.Cell(row, 10).Value = x.Departamento1;
                ws.Cell(row, 11).Value = x.Departamento2;
                ws.Cell(row, 12).Value = x.NombrePer;
                ws.Cell(row, 13).Value = x.DocPer;
                ws.Cell(row, 14).Value = x.TelfPer;
                ws.Cell(row, 15).Value = x.Cajas;
                ws.Cell(row, 16).Value = x.Peso;
                ws.Cell(row, 17).Value = x.Transportista;
                ws.Cell(row, 18).Value = x.ModoEnvio;
